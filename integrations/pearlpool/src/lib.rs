use stratum::{PoolAdapter, JsonRpcRequest, JsonRpcResponse};
...
impl PoolAdapter for PearlPoolAdapter {
    ...
    fn parse_difficulty(&self, request: &JsonRpcRequest) -> Option<f64> {
        if request.method == "mining.set_difficulty" {
            request.params.as_array()?.get(0)?.as_f64()
        } else {
            None
        }
    }
    ...
    fn build_share_submit(&self, share: &ShareCandidate) -> JsonRpcRequest {
        JsonRpcRequest {
            ...
            params: json!([share.worker, share.job_id, "0x00000000", share.timestamp.to_rfc3339(), share.nonce]),
        }
    }
