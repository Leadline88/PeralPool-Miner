...
impl StratumClient {
    ...
    pub async fn send_request(&self, mut req: JsonRpcRequest) -> Result<JsonRpcResponse, StratumError> {
        ...
        if let Some(tx) = message_tx {
            tx.send(StratumMessage::Request(req)).map_err(|_| StratumError::ConnectionClosed)?;
        } else {
