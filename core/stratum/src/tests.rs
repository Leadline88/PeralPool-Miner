...
mod tests {
    ...
    impl PoolAdapter for TestAdapter {
        ...
        fn parse_subscribe_response(&self, _res: &crate::protocol::JsonRpcResponse) -> Result<(), String> { Ok(()) }
        fn build_login_request(&self, wallet: &str, worker: &str) -> crate::protocol::JsonRpcRequest {
            crate::protocol::JsonRpcRequest {
                id: None,
        ...
        fn parse_login_response(&self, _res: &crate::protocol::JsonRpcResponse) -> Result<bool, String> { Ok(true) }
        fn parse_job(&self, req: &crate::protocol::JsonRpcRequest) -> Option<serde_json::Value> {
            if req.method == "mining.notify" { Some(req.params.clone()) } else { None }
        }
        fn parse_difficulty(&self, req: &crate::protocol::JsonRpcRequest) -> Option<f64> {
            if req.method == "mining.set_difficulty" { req.params.as_array()?.get(0)?.as_f64() } else { None }
        }
        ...
        fn parse_share_response(&self, _res: &crate::protocol::JsonRpcResponse) -> Result<bool, String> { Ok(true) }
    }
    ...
    async fn test_full_pipeline() {
        ...
        let miner = MinerLoop::new(client.clone(), adapter, "wallet".to_string(), "worker".to_string());
